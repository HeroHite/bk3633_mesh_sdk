...
static void hb_send(struct bt_mesh_model *model)
{
    ...
    /*modified for NODE/CFG/HBP/BV-05-C*/
    if (bt_mesh.lpn.state >= BT_MESH_LPN_ESTABLISHED) {
...
static u8_t mod_unbind(struct bt_mesh_model *model, u16_t key_idx)
{
    ...
    BT_DBG("model %04x key_idx 0x%04x", model->id, key_idx);
...
s8_t genie_mesh_setup(void)
{
    ...
    for(i = 0; i < elem_cnt; i++){
        ...
        if(!elem) {
            BT_ERR("can not find ele for addr %d", primary_addr + i);
            return -1;
        }
        ...
        for (j = 0; j < elem->model_count; j++) {
            ...
            memcpy(mod->groups, g_sub_list, sizeof(g_sub_list));
        }
...
static void _elem_sub_bind(struct bt_mesh_elem *p_elem)
{
    ...
    for (i = 0; i < p_elem->vnd_model_count; i++) {
        p_mod = bt_mesh_model_find_vnd(p_elem, p_elem->vnd_models[i].vnd.company, p_elem->vnd_models[i].vnd.id);
        memcpy(p_mod->groups, g_sub_list, sizeof(g_sub_list));
    }
}
...
void lpn_send_hb(void)
{
    ...
    if ((conf->hb_pub.feat & BT_MESH_FEAT_LOW_POWER) && sub) {
        hb_send(conf->model);
    }
}
#endif
